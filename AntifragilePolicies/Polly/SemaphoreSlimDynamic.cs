...
namespace AntifragilePolicies.Polly
{
    public class SemaphoreSlimDynamic : SemaphoreSlim
    {
        ...
        internal void AdjustConcurrency(int newLimit)
        {
            if(newLimit < AvailableSlotsCount)
            {
                var decreaseCount = this.AvailableSlotsCount - newLimit;
                TryDecrease(decreaseCount);
            }
            else
            {
                var increaseCount = newLimit -  this.AvailableSlotsCount;
                TryIncrease(increaseCount);
            }
        }
